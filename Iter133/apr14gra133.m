...
load DRF.mat;
load apr14LayerAvg.mat
Clayer=TotalCbyLayer;
...
ycNW = 80;
   ...
   size(VH)
   size(repmat(Vlayer,NG))
   size(recipH)
   
   resid1 = (-VH - Vlayer.*H.*DYinG').*(NH - Clayer.*H).*recipH;
...
lenE = ycNE-ycSE+1;
   ...
   resid3 = (VH - Vlayer.*H.*DYinG').*(NH - Clayer.*H).*recipH;
   size(resid3)
   % this guy has to change to V1_y
   %V1_y = VH1_y.*(recipArea1); % weighted average v
...
H = VH;
 ...
 NH = VH;
   ...
   resid2= (VH - Vlayer.*H.*DXinG).*(NH - Clayer.*H).*recipH;
   size(resid3);
save('apr14resids.mat', 'resid1', 'resid2','resid3'); 
...
Vlayer = TotalVHbyLayer./(Area1+Area2+Area3);
...
%save('apr14LayerAvg.mat', 'Vlayer', 'TotalCbyLayer');
%%
figure(3)
