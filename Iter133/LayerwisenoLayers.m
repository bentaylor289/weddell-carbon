...
densityBounds = [23 29];%27.5 27.7 27.9 28 28.1 28.2 28.25 28.3 28.4 28.5 29];
...
% xcW and xcE are defined at the beginning
   ...
   G = squeeze(ncread(strcat(folder, 'GAMMA.nc'),'gamma', [1 ycN-1 1 t], [Inf 2 Inf 1]));  % this is the one we like. 
   V = squeeze(ncread(strcat(folder, 'Vvel.nc'), 'VVEL' , [1 ycN 1 t], [Inf 1 Inf 1])); %rdmds([fpath 'OUTPUT_3dy/diag_state'],ts(t),'rec',4);
   N = squeeze(ncread(strcat(folder, 'DIC.nc'), 'TRAC01', [1 ycN-1 1 t], [Inf 2 Inf 1])); %rdmds([fpath 'OUTPUT_3dy/diag_bgc'],ts(t),'rec',4
  
   mask = squeeze(ncread(strcat(folder, 'DIC.nc'), 'hFacC', [1 ycN-1 1 ], [Inf 2 Inf])); 
   Hinz = squeeze(ncread(strcat(folder, 'Vvel.nc'), 'hFacS', [1 ycN 1], [Inf 1 Inf]));
   DXinG = squeeze(ncread(strcat(folder, 'Vvel.nc'), 'dxG', [1 ycN], [Inf 1]));       
   Hinz = Hinz.*DRF';
...
save('LayerwiseTest'+string(tf)+'.mat', 'VH1_y','VH2_x','VH3_y', 'N1_y', 'N2_x', 'N3_y','Area1','Area2','Area3','NVH1','NVH2','NVH3', 'cont1' , 'cont2' , 'cont3','NVHtot', 'OVRtot', 'REStot', 'VOLtot')
