...
gammaM = gammaM/(wkend-wkstart+1) + 1e-10*rand(bla(2), yc-1,52).*hF;% random noise added to prevent identical neutral density; in general its a problem that neutral density is not strictly increasing, and I need another algorithm. 
...
gammaF = gammaF/(wkend-wkstart+1)+ 1e-10*rand(bla(2), yc-1,52).*hF;
...
for xk = 1:(6*(360+lonE-lonW))
	for yk = 1:(yc-1)
		if (yk>77 && isINS(xk,yk-77)==1)
 			gs = sum(hF(xk,yk,:));
			startDeps(xk,yk,:) = interp1(squeeze(gammaM(xk,yk,1:gs)), depth(1:gs), qGams);
 			finDeps(xk,yk,:) = interp1(squeeze(gammaF(xk,yk,1:gs)), depth(1:gs), qGams);
end
end
end
...
for xk = 1:(6*(360+lonE-lonW))
 ...
 gs = sum(hF(xk,yk,:));
 if gs>3
 Allstart(xk,yk,:) = interp1(squeeze(gammaM(xk,yk,1:gs)), depth(1:gs), qGams);
 Allfin(xk,yk,:) = interp1(squeeze(gammaF(xk,yk,1:gs)), depth(1:gs), qGams);
end
end
end
