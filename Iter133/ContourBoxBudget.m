...
for k=1:maxlen-1
    ...
    % fairly obscure formula to find the area contained by the contour in the boundary grid cells
    if (l==1)
    x1 = xx(k)- xk+1; x2 = xx(k+1) - xk+1; 
    y1 = yy(k)- yk+1; y2 = yy(k+1) - yk+1;
    A = abs((x2-x1)*(y2-y1))/2;
    if (x1<=x2 & y1<=y2)
        Ak = A+1+(y1-1)*x2;
    elseif (x1<=x2 & y1 > y2)
...
%%
save('WeddellContTEST'+string(monstart)+'to'+string(monend)+'.mat', 'isINS','tendTot', 'dilutTot', 'advTot', 'corrTot', 'surfTot','bioTot', 'mixTot','resTot');
