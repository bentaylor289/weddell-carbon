...
advTot(l) = -sum(sum(adv_tosum(xcFull,:).*hFac.*DXG(xcFull,yc).*repmat(DRF,[1 498])')); % because this is advection to the north!
...
SSHin = ncread(strcat(budgetfolder, 'SSH.nc'), 'ETAN', [1 1 t], [Inf Inf 2] );
...
%%
save('WeddellVol'+string(monstart)+'to'+string(monend)+'REDO.mat', 'SSHTot', 'FWTot', 'advTot', 'Volres');
